        ...
        <div className="Game">
            ...
            {chords.length > 0 &&
                <div className="Game__answer">
                    <span className={message && 'Game__error-message'}>{!message ? Object.values(answerObj).map(v => v[0]).join(' ') : message}</span>
                </div>
            }

            <Button onClickHandler={() => chords.length > 0 ? onClickHandler(chords[0].name) : onClickBack()} text={chords.length > 0 ? "Check" : "Back"} className="btn-answer" />
